...
function start()
{
    ...
    let tailLength = 9
    getVisitedGrid(data, tailLength)
    console.log(trackerCounter.size)
}

function getVisitedGrid(data, tailLength)
{
    ...
    {
        T[i] = new Position(0,0)
    }

    let tailTracker = []

    AddTailPosition(tailTracker, T[tailLength-1])
    ...
    data.forEach(step => {
        ...
        for(let i = 0; i < steps; i++)
        {
            ...
            let tailMovement = CheckRequiredMovement(H, T[0])
            T[0].MakeMovement(tailMovement)
            for(let i = 1; i < tailLength; i++)
            {
                tailMovement = CheckRequiredMovement(T[i-1], T[i])
                T[i].MakeMovement(tailMovement)
            }
            AddTailPosition(tailTracker, T[tailLength-1])
        }
    })

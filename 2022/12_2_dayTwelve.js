...
function start() {
  ...
  findStepsToClosestElevationA(adjacencyList, end, map)
...
function findStepsToClosestElevationA(adjacencyList, end, map)
{
  let visited = new Set();
  let startPoint = end.join(',')
  ...
  while (queue.length > 0) {
    ...
    {
      let locationCoordinates = location.split(',').map(Number)
      let currentElevation = map[locationCoordinates[0]][locationCoordinates[1]]

      if(currentElevation == target)
      {

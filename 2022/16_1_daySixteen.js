...
function processData(data) {
    ...
    return getMaxFlow(data, currentPipe, opened, timeRemaining, maxFlow, DP)    
}
...
function getMaxFlow(data, currentPipe, opened, timeRemaining, maxFlow, DP, path = []) {

    if((currentPipe == 'DD' && timeRemaining == 28+1) || (currentPipe == 'BB' && timeRemaining == 25+1) || (currentPipe == 'JJ' && timeRemaining == 21+1))
    {
        console.log('Hi')
    }
    ...
    for (let i = 0; i < targets.length; i++) {
        ...
        let result = DP[targets[i] + ',' + timeRemaining - steps]
        if(result != null)
        {
            allFlows.push(result)
        }
        else{
            let gotten = getMaxFlow(data, targets[i], new Set(opened), timeRemaining - steps, maxFlow, DP, [...path])
            allFlows.push(gotten)
            DP[targets[i] + ',' + timeRemaining] = gotten
        }
    }

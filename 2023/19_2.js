/*
Learnings: 
First thought, this might work with BFS where I find everything that finally results in A.
Finally, I did use something like that with ranges. Made a mistake with ONE 1 and broke my head for 2 days believing I was stupid.
Man. Trust yourself more
*/

let dummyInput = `px{a<2006:qkq,m>2090:A,rfg}
pv{a>1716:R,A}
lnx{m>1548:A,A}
...
let actualInput = `vdm{m<3285:R,R}
...
// var FindAllRanges = function(instructionSet, command)
// {
//     let instructions = instructionSet[command]
//     let ranges = [], invertRanges = []
//     let i = 0
...
//         {
//             let [range, result] = instruction.split(':')
//             if(result == 'A') ranges.push(range)
//             else if(result != 'R')
//             {
//                 let [r, ir] = FindAllRanges(instructionSet, result)
//                 ranges.push(...r)
//                 invertRanges.push(...ir)
//             }
//         }
//         else
//         {
//             if(instruction == 'A') invertRanges.push(...instruction)
//             else if(instruction != 'R') 
//             {
//                 let [r, ir] = FindAllRanges(instructionSet, instruction)
//                 ranges.push(...r)
//                 invertRanges.push(...ir)
//             }
//         }
...
//     return [ranges, invertRanges]
// }
let map = {}

var FindPossibleValues = function(ranges)
{
    // console.log(ranges)
    ProcessData(map, ranges, false)
    let result = ProcessRanges(ranges)
    // console.log(result)
    return result
}

var ProcessRanges = function(ranges)
{
    let totalPossibilities = 1
    for(let letter of 'xmas')
    {
        let letterWiseRange = ranges.filter(x => x.indexOf(letter) > -1)
        if(letterWiseRange.length == 0)
        {
            totalPossibilities *= 4000
            continue
        }

        let greaterNumber = letterWiseRange.filter(x => x.indexOf('>') > -1).sort((m,a) => parseInt(m.split('>')[1]) - parseInt(a.split('>')[1])).pop()
        let lesserNumber = letterWiseRange.filter(x => x.indexOf('<') > -1).sort((m,a) => parseInt(a.split('<')[1]) - parseInt(m.split('<')[1])).pop()

        // Easier to debug hence
        greaterNumber = greaterNumber == undefined ? null : parseInt(greaterNumber.split('>')[1])
        lesserNumber = lesserNumber == undefined ? null : parseInt(lesserNumber.split('<')[1])

        if(greaterNumber != undefined && lesserNumber !=undefined && lesserNumber > greaterNumber)
        {
            totalPossibilities *= lesserNumber - greaterNumber - 1
        }
        else if(greaterNumber == undefined)
        {
            totalPossibilities *= lesserNumber - 1
        }
        else if(lesserNumber == undefined)
        {
            totalPossibilities *= 4000 - greaterNumber
        }
        // else if(lesserNumber == undefined && greaterNumber == undefined)
        // {
        //     totalPossibilities *= 4000
        // }
        else
        {
            console.log('YOOOOOOO WTFFFF')
        }
    }
    return totalPossibilities
}

var ProcessData = function(map, ranges, isInvert)
{
    for(let letter of 'xmas')
    {
        let greater = [], lower = []
        ...
        if(letterWiseRange.length > 0)
        {
            if(!isInvert)
            {
                lower.push(...letterWiseRange.filter(x => x.indexOf('<') > -1))
                greater.push(...letterWiseRange.filter(x => x.indexOf('>') > -1))
            }
            else
            {
                lower.push(...letterWiseRange.filter(x => x.indexOf('>') > -1).map(x => x.split('>')[0] + '<' + (parseInt(x.split('>')[1]) - 1)))
                greater.push(...letterWiseRange.filter(x => x.indexOf('<') > -1).map(x => x.split('<')[0] + '>' + (parseInt(x.split('<')[1]) + 1)))
            }
        }
        ...
        if(map[letter]['>'] == undefined)  map[letter]['>'] = []
        if(map[letter]['<'] == undefined)  map[letter]['<'] = []
        map[letter]['>'].push(...greater)
        map[letter]['<'].push(...lower)
    }

}

var GetInvertedPredicate = function(range)
{
    let [letter, number] = range.split(/[<>]/)
...
var FindAllRanges = function(instructionSet, command, ranges = [])
{
    let instructions = instructionSet[command]
    ...
    while(instructions.length > i)
    {
        ...
        if(instruction.indexOf(':') > -1) 
        {
            ...
            if(result == 'A') 
            {
                totalPossibilities += FindPossibleValues([...ranges, ...newR])
                newIR.push(GetInvertedPredicate(range))
            }
            else if(result != 'R')
            {
                totalPossibilities += FindAllRanges(instructionSet, result, [...ranges, ...newR])
                newIR.push(GetInvertedPredicate(range))
            }
            else if(result == 'R') newIR.push(GetInvertedPredicate(range))
        }
        else
        {
            if(instruction == 'A') 
            {
                totalPossibilities += FindPossibleValues(ranges)
            }
            else if(instruction != 'R') 
            {
                totalPossibilities += FindAllRanges(instructionSet, instruction, ranges)
            }
        }
        i++
        ranges.push(...newIR)
...
var FormatInput = function(input)
{
    ...
    for(let instruction of instructions.replaceAll('}','').split('\n'))
    {
        let [code, checks] = instruction.split('{')
        checks = checks.split(',')
        instructionSet[code] = checks
    }
...
{
    let instructionSet = FormatInput(input)
    let result = FindAllRanges(instructionSet, "in")
    return result
}

...
let actualInput = `Game 1: 7 blue, 9 red, 1 green; 8 green; 10 green, 5 blue, 3 red; 11 blue, 5 red, 1 green
...
var findSumOfPowers = function(data)
{
    let powers = []
    for(let game of data)
    {
        let [gameNumber, cubeData] = game.split(': ')
        powers.push(findPowerOfCubes(cubeData))
    }
    let sumOfPowers = powers.reduce((a,b) => a+b)
    return sumOfPowers
}

var findPowerOfCubes = function(cubeData)
{
    let totalNumberOfCubes = {
        red : 0,
        green : 0,
        blue : 0
    }
    let sets = cubeData.split('; ')
    for(let set of sets)
    {
        let cubes = set.split(', ')
        for(let cube of cubes)
        {
            let [numberOfCubes, colorOfCube] = cube.split(' ')
            if(totalNumberOfCubes[colorOfCube] < parseInt(numberOfCubes))
            {
                totalNumberOfCubes[colorOfCube] = parseInt(numberOfCubes)
            }
        }
    }
    return totalNumberOfCubes.red * totalNumberOfCubes.green * totalNumberOfCubes.blue
}

var formatInput = function(input)
{
    let data = input.replaceAll('Game ','').split('\n')
    return findSumOfPowers(data)
}

/*
Learnings: 
Sometimes failing teaches you something SO MUCH BETTER

Learnt : Ray Casting Algorithm

Caching ie Dynamic Programming can help here by a lot.
...
let dummyInput4 = `FF7FSF7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
F--JF--7||LJLJ7F7FJ-
L---JF-JLJ.||-FJLJJ7
...
let actualInput = `F--FJ77F-L7---J-F777-F7|77F|-J-F|77|F-L-J-77-L7FL77-7|----.F7-|-|.7-.-FL-FF-FF77.|.F-F-.F|.7.F.7-F-77.J7|----7LL7F|7|-F-J-777-77.F-7-FL77FF|
...
var formAdjacencyList = function(data)
{
    for (let i = 0; i < data.length; i++) {
        for (let j = 0; j < data[i].length; j++) {
            let position = i + ',' + j
            adjacencyList.set(position, [])            
            if(j + 1 < data[i].length && pipes[data[i][j+1]].has('W') && pipes[data[i][j]].has('E')) 
                adjacencyList.get(position).push(i+','+(j+1))

            if(j - 1 >= 0 && pipes[data[i][j-1]].has('E') && pipes[data[i][j]].has('W')) 
                adjacencyList.get(position).push(i+','+(j-1))

            if(i + 1 < data.length && pipes[data[i+1][j]].has('N') && pipes[data[i][j]].has('S')) 
                adjacencyList.get(position).push((i+1)+','+j)

            if(i - 1 >= 0 && pipes[data[i-1][j]].has('S') && pipes[data[i][j]].has('N')) 
                adjacencyList.get(position).push((i-1)+','+j)
            
            if(data[i][j] == 'S') startingPosition = position
        }
    }
}

var isPointEnclosed = function()
{
    let queue = [[startingPosition, 0, new Set(), '']]
    while(queue.length > 0)
    {
        let [currentPosition, distance, visited, map] = queue.shift()
        distance = parseInt(distance) + 1
        visited.add(currentPosition)
        let directions = adjacencyList.get(currentPosition)
        for(let direction of directions)
        {
            if(!visited.has(direction))
            {
                queue.push([direction, distance, new Set(visited), map+'->'+currentPosition])
            }
            else if(direction == startingPosition && distance > 2)
            {
...
var findEnclosedSpaces = function(data, set, maxHeight)
{
    let enclosedCount = 0
    for (let i = 0; i < data.length; i++) {
        for (let j = 0; j < data[i].length; j++) {
            if(!set.has(i+','+j))
            {
                let isOutside = raycast(data, i, j, set, maxHeight)
                if(!isOutside)
                {
                    console.log(i + ' ' + j + ' : ' + isOutside)
                    enclosedCount++
                }
            }
        }
...
var replaceS = function(data)
{
    let [x, y] = startingPosition.split(',').map(Number)
    let directions = ''
    if(x + 1 < data.length)
        if('|LJ'.indexOf(data[x+1][y]) > -1) directions += 'S'
    if(x - 1 >= 0)
        if('|F7'.indexOf(data[x-1][y]) > -1) directions += 'N'
    if(y + 1 < data[0].length)
        if('-7J'.indexOf(data[x][y+1]) > -1) directions += 'E'
    if(y - 1 >= 0)
        if('-FL'.indexOf(data[x][y-1]) > -1) directions += 'W'

    switch(directions)
    {
        case 'SN' : data[x][y] = '|'; break;
        case 'SE' : data[x][y] = 'F'; break;
        case 'SW' : data[x][y] = '7'; break;
        case 'NE' : data[x][y] = 'L'; break;
        case 'NW' : data[x][y] = 'J'; break;
        case 'EW' : data[x][y] = '-'; break;
    }
}

var raycast = function(data, i, j, set, maxHeight)
{
    let interestedCheckers = new Set(['F','L','-'])
    let index = i, intercepts = 0
    if(i == 4 && j == 11)
    {
        console.log('hi')
    }
    ...
    {
        if(set.has(index+','+j) && interestedCheckers.has(data[index][j])) intercepts++
        index++
    }
    
    return intercepts % 2 == 0
}
...
var formatInput = function(input)
{
    ...
    formAdjacencyList(data)
    let map = isPointEnclosed()
    let set = new Set(map.split('->').filter(x => x!=''))
    replaceS(data)
    return findEnclosedSpaces(data, set, maxHeight)
}

// console.log('Dummy Output 1 : ' + formatInput(dummyInput))
// console.log('Dummy Output 2 : ' + formatInput(dummyInput2))
// console.log('Dummy Output 3 : ' + formatInput(dummyInput3))
// console.log('Dummy Output 4 : ' + formatInput(dummyInput4))

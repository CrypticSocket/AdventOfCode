/*
Learnings: 
This wasn't too hard tbh. I BY CHANCE remembered I can use eval and used that here.
However, if not for that I had an idea to make this more efficient by combining all the possible ranges into one condition.
I think that would have helped a lot with part 2
*/
...
var RunChecks = function(data)
{
    let [instructionSet, partParameters] = data

    let A = []

    while(partParameters.length > 0)
    {
        let [x,m,a,s] = partParameters.shift()
        let command = 'in'
        while(true)
        {
            for(let instruction of instructionSet[command])
            {
                ...
                let [check, truthValue] = instruction.split(':')
                if(eval(check))
                {
                    command = truthValue
                    break;
...
{
    [instructions, partParameters] = input.split('\n\n')
    partParameters = partParameters.replaceAll(/[{}=xmas]/g, '').split('\n').map(x => x.split(',').map(Number))
    let instructionSet = {}
    ...
    return [instructionSet, partParameters]
}

var GetResult = function(input)
{
    let data = FormatInput(input)
    let result = RunChecks(data)
    return result
}
